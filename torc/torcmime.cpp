...
{
    if (Device)
        return QMimeDatabase().mimeTypeForData(Device).name();
    return QString();
}
...
QString TorcMime::MimeTypeForFileNameAndData(const QString &FileName, QIODevice *Device)
{
    ...
    return QMimeDatabase().mimeTypeForFileNameAndData(FileName, Device).name();
}
...
QString TorcMime::MimeTypeForFileNameAndData(const QString &FileName, const QByteArray &Data)
{
    ...
    return QMimeDatabase().mimeTypeForFileNameAndData(FileName, Data).name();
}
...
QStringList TorcMime::MimeTypeForFileName(const QString &FileName)
{
    ...
    QList<QMimeType> types = QMimeDatabase().mimeTypesForFileName(FileName);
...
QStringList TorcMime::ExtensionsForType(const QString &Type)
{
    ...
    QList<QMimeType> mimes = QMimeDatabase().allMimeTypes();

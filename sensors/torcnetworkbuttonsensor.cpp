...
/*! \class TorcNetworkButtonSensor
 *  \brief A network input that mimics the behaviour of a mechanical push button.
 *
 ...
 * \note Linking this device to a network button output is unlikely to generate useful behaviour.
 * \sa TorcNetworkButtonOutput
*/
TorcNetworkButtonSensor::TorcNetworkButtonSensor(double Default, const QVariantMap &Details)
...
TorcNetworkButtonSensor::~TorcNetworkButtonSensor()
{
}

QStringList TorcNetworkButtonSensor::GetDescription(void)
{
...
*/
void TorcNetworkButtonSensor::SetValue(double Value)
{
    (void)Value;
    ...
    TorcSwitchSensor::SetValue(value < 1.0 ? 1.0 : 0.0);
    emit Pushed();
}

void TorcNetworkButtonSensor::StartTimer(void)
{
    if (!m_pulseTimer.isActive())
        m_pulseTimer.start();
}

void TorcNetworkButtonSensor::EndPulse(void)
{
    QMutexLocker locker(lock);
